...
static doca_error_t create_egress_root_rss(struct doca_flow_port *port, struct doca_flow_pipe **pipe){
    struct doca_flow_pipe_cfg *cfg;
 ...
 doca_error_t flow_rss_meta_with_app_buffering(int nb_queues)
 {
    ...
    //  r = init_doca_flow(nb_queues, "vnf,hws", &res, nr_shared);
     r = init_doca_flow(nb_queues, "switch,hws", &res, nr_shared);
     if (r != DOCA_SUCCESS) {
     ...
     for (port_id = 0; port_id < nb_ports; port_id++) {
         ...
         r = create_egress_root_rss(ports[port_id], &pipe);
         if (r != DOCA_SUCCESS) { DOCA_LOG_ERR("create_egress_root_rss failed"); goto stop_ports; }
 
         r = doca_flow_entries_process(ports[port_id], 0, DEFAULT_TIMEOUT_US, 1);
         if (r != DOCA_SUCCESS) { DOCA_LOG_ERR("entries_process failed"); goto stop_ports; }
     }
