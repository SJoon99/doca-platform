...
이 문서는 `doca-platform` 레포지토리의 디렉토리 구조를 설명하고, 각 디렉토리가 어떤 아키텍처 컴포넌트에 해당하는지 매핑합니다.
...
Kubernetes API 서버에 등록되는 모든 커스텀 리소스의 Go 타입 정의. 하위 디렉토리 각각이 별도의 API 그룹입니다.
...
하위 디렉토리 각각에 배포 가능한 바이너리 하나의 `main.go`가 있습니다.
...
컴포넌트별로 구성된 Kubernetes 배포 매니페스트. 각 하위 디렉토리는 Kubebuilder 표준 레이아웃(`default/`, `manager/`, `rbac/`, `webhook/`, `certmanager/`)을 따릅니다.
...
각 DOCA 서비스에 대한 즉시 사용 가능한 `DPUService` / `DPUDeployment` 매니페스트. 호스트 클러스터에 적용하면 DPF가 ArgoCD를 통해 DPU 클러스터로 자동 동기화합니다.
...
핵심 비즈니스 로직. 외부 Go 모듈에서 import 불가.
